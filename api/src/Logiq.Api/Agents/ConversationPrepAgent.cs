    ...
    ILogger<ConversationPrepAgent> logger) : IConversationPrepAgent
{
    ...
    public async Task<ConversationPrep> PrepareAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        logger.LogInformation("Preparing 1:1 conversation brief for member {MemberId} in team {TeamId}", memberId, teamId);

        var member = await employeeRepository.GetByIdAsync(teamId, memberId, cancellationToken);
        var meetings = await hrTools.ListMeetings(teamId, cancellationToken);
        var deferred = await hrTools.GetDeferredTopics(teamId, cancellationToken);

        var memberWellbeing = wellbeingAnalysis.RiskSignals.FirstOrDefault(r => r.EmployeeId == memberId);
        var memberSkills = skillsAnalysis.EmployeeInsights.FirstOrDefault(e => e.EmployeeId == memberId);
        var memberDelivery = deliveryAnalysis.EmployeeInsights.FirstOrDefault(e => e.EmployeeId == memberId);
        ...
        var prep = TryParsePrep(responseText, teamId, memberId, member?.Name ?? "Unknown");
    ...
    private async Task PersistPrepToDashboard(string teamId, string memberId, ConversationPrep prep, CancellationToken cancellationToken)
    {
        var existing = await memberRepository.GetDashboardAsync(teamId, memberId, cancellationToken);
    ...
    private static ConversationPrep TryParsePrep(string json, string teamId, string memberId, string memberName)
    {
        try
        {
            var result = JsonSerializer.Deserialize<ConversationPrep>(json,
    ...
    private static ConversationPrep FallbackPrep(string teamId, string memberId, string memberName) => new()
    {
        ...
        SuggestedTopics = ["Check in on workload and wellbeing", "Discuss development goals", "Review sprint performance"],
