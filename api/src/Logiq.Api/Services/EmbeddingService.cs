    ...
    ILogger<EmbeddingService> logger) : IEmbeddingService
{
    public async Task<ReadOnlyMemory<float>?> GetEmbeddingAsync(string text, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            ClientResult<OpenAIEmbeddingCollection>? response = await embeddingClient.GenerateEmbeddingsAsync([text], cancellationToken: cancellationToken).ConfigureAwait(false);
