...
public sealed class MeetingRepository(IOptions<StorageOptions> options) : IMeetingRepository
{
    ...
    public async Task<IReadOnlyList<Meeting>> ListUpcomingAsync(string teamId, CancellationToken cancellationToken = default)
    {
        await _meetings.CreateIfNotExistsAsync(cancellationToken);
        var results = new List<Meeting>();
        await foreach (var entity in _meetings.QueryAsync<MeetingEntity>(e => e.PartitionKey == teamId && !e.IsPast, cancellationToken: cancellationToken))
            results.Add(MapToModel(entity));
        return results;
    }

    public async Task<IReadOnlyList<Meeting>> ListPastAsync(string teamId, CancellationToken cancellationToken = default)
    {
        await _meetings.CreateIfNotExistsAsync(cancellationToken);
        var results = new List<Meeting>();
        await foreach (var entity in _meetings.QueryAsync<MeetingEntity>(e => e.PartitionKey == teamId && e.IsPast, cancellationToken: cancellationToken))
    ...
    public async Task<Meeting?> GetByIdAsync(string teamId, string meetingId, CancellationToken cancellationToken = default)
    {
        await _meetings.CreateIfNotExistsAsync(cancellationToken);
        try
        {
            var response = await _meetings.GetEntityAsync<MeetingEntity>(teamId, meetingId, cancellationToken: cancellationToken);
    ...
    public async Task<IReadOnlyList<DeferredTopic>> ListDeferredTopicsAsync(string teamId, CancellationToken cancellationToken = default)
    {
        await _deferred.CreateIfNotExistsAsync(cancellationToken);
        var results = new List<DeferredTopic>();
        await foreach (var entity in _deferred.QueryAsync<TableEntity>(e => e.PartitionKey == teamId, cancellationToken: cancellationToken))
        {
    ...
    public async Task UpsertAsync(string teamId, Meeting meeting, bool isPast, CancellationToken cancellationToken = default)
    {
        await _meetings.CreateIfNotExistsAsync(cancellationToken);
    ...
    public async Task UpsertDeferredTopicAsync(string teamId, DeferredTopic topic, CancellationToken cancellationToken = default)
    {
        await _deferred.CreateIfNotExistsAsync(cancellationToken);
        var entity = new TableEntity(teamId, topic.Id)
        {
            ["Text"] = topic.Text,
            ["Person"] = topic.Person
        };
