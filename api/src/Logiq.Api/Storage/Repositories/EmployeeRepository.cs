...
public sealed class EmployeeRepository(IOptions<StorageOptions> options) : IEmployeeRepository
{
    ...
    public async Task<IReadOnlyList<Employee>> ListByTeamAsync(string teamId,
        CancellationToken cancellationToken = default)
    {
        await _table.CreateIfNotExistsAsync(cancellationToken);
        List<Employee> results = [];
        await foreach (EmployeeEntity entity in _table.QueryAsync<EmployeeEntity>(e => e.PartitionKey == teamId,
                           cancellationToken: cancellationToken))
            results.Add(MapToModel(entity));
        return results;
    }

    public async Task<Employee?> GetByIdAsync(string teamId, string employeeId,
        CancellationToken cancellationToken = default)
    {
        await _table.CreateIfNotExistsAsync(cancellationToken);
        try
        {
            Response<EmployeeEntity> response =
                await _table.GetEntityAsync<EmployeeEntity>(teamId, employeeId, cancellationToken: cancellationToken);
    ...
    {
        await _table.CreateIfNotExistsAsync(cancellationToken);
        EmployeeEntity entity = MapToEntity(teamId, employee);
