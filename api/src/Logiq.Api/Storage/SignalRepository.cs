...
public sealed class SignalRepository(IOptions<StorageOptions> options) : ISignalRepository
{
    ...
    public async Task<IReadOnlyList<Signal>> ListTeamSignalsAsync(string teamId, CancellationToken cancellationToken = default)
    {
        await _table.CreateIfNotExistsAsync(cancellationToken);
        var results = new List<Signal>();
        await foreach (var entity in _table.QueryAsync<SignalEntity>(e => e.PartitionKey == teamId && !e.IsMemberSignal, cancellationToken: cancellationToken))
            results.Add(MapToSignal(entity));
        return results;
    }

    public async Task<IReadOnlyList<MemberSignal>> ListMemberSignalsAsync(string teamId, string memberId, CancellationToken cancellationToken = default)
    {
        await _table.CreateIfNotExistsAsync(cancellationToken);
        var partitionKey = $"{teamId}:{memberId}";
        var results = new List<MemberSignal>();
        await foreach (var entity in _table.QueryAsync<SignalEntity>(e => e.PartitionKey == partitionKey && e.IsMemberSignal, cancellationToken: cancellationToken))
    ...
    public async Task UpsertMemberSignalAsync(string teamId, string memberId, MemberSignal signal, CancellationToken cancellationToken = default)
    {
        await _table.CreateIfNotExistsAsync(cancellationToken);
        var entity = new SignalEntity
        {
